...
**第二轮 Spec 轴（`del_mu3bub3w_9hye`）：一条待产品拍板的口径**

- **「不会静默重登成新账号」跨冷启动不成立**（Spec 轴，中）：本进程内成立（`accountDeleted` 早退 + `session.logout()`），但下一次冷启动读不到 token（`session.ts:96-104`）仍会 `signInOnce()`（`session.ts:147-151`），同一 openid 被服务端建成新的空 user/family。当前口径（`AccountGone.vue` 文案「下次进入小程序会开始一个新的空账号」）与实现一致，但把验收第 4 条解释成「注销成功后回到干净初始状态」。**若要字面兑现该验收项**，需要另开一条实现：注销时在本地落一个「已注销」标记（`uni.setStorageSync`），冷启动读到就先展示 `AccountGone`，由家长主动点「重新开始」才登录新建账号。这是个产品口径选择（多一次点击 vs 字面承诺），已提请本项目负责人拍板，未在本票擅自改实现。
