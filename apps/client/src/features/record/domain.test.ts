...
describe('记录领域契约', () => {
  it('与后端字段一致并允许不填奶量', () => {
    ...
    expect(validatePayload('vitamin_ad', { kind: 'vitamin_ad', dose_text: null })).toBeNull()
    expect(describeRecord({ record_type: 'vitamin_ad', payload: { kind: 'vitamin_ad', dose_text: '1 滴' } })).toBe('已吃 · 1 滴')
