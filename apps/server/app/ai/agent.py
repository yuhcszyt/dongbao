...
def _fallback_without_model(scope: BabyScope, message: str) -> ParentingAnswer:
    ...
    if knowledge and isinstance(knowledge[0], dict) and knowledge[0].get("error"):
        return ParentingAnswer(
            summary="智能问答与向量检索尚未完整配置。",
            reasons=[str(knowledge[0]["error"]), "聊天需要 MODEL_API_KEY；知识库检索需要 EMBEDDING_API_KEY（与 DeepSeek 聊天 key 分开）。"],
            baby_context=[f"昵称 {profile.get('nickname')}"],
            actions=["在 .env 配置腾讯云 TokenHub 的 EMBEDDING_API_KEY 后执行 make seed-rag", "配置 MODEL_API_KEY 后获得完整回答"],
            watch_for=[],
            sources=[],
            related_record_ids=[],
        )
